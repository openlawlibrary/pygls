...
async def goto(get_client_for):
    client, response = await get_client_for("goto.py")
...
async def test_declaration(
    ...
    """Ensure that we can implement declaration requests."""
    client, initialize_result = goto

    declaration_options = initialize_result.capabilities.declaration_provider
    assert declaration_options is True
    ...
    response = await client.text_document_declaration(
    ...
    response = await client.text_document_declaration(
        types.DeclarationParams(
            text_document=types.TextDocumentIdentifier(uri=test_uri),
            position=types.Position(line=5, character=52),
        )
    )
    ...
    assert response.range == types.Range(
        ...
        end=types.Position(line=5, character=25),
    )
