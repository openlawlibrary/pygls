...
class JsonRPCHandler:
    """JSON-RPC message handler based on asyncio."""
    ...
    def __init__(
        ...
        *,
        ...
        self._futures: Dict[json_rpc.MsgId, asyncio.Future[Any]] = {}
        self._tasks: Dict[json_rpc.MsgId, asyncio.Task[Any]] = {}
    ...
    def __call__(self, data: bytes) -> Any:
        ...
        # Generate a message id if required - that way we can reference any task by id.
        msg_id: json_rpc.MsgId = getattr(message, "id", None) or self.protocol.make_id()
        msg_type = type(message).__name__

        if isinstance(message, json_rpc.JsonRPCRequestMessage):
            coro = self._handle_request(message.id, message.method, message.params)

        elif isinstance(message, json_rpc.JsonRPCNotification):
            coro = self._handle_notification(message.method, message.params)

        elif isinstance(message, json_rpc.JsonRPCResultMessage):
            coro = self._handle_result(message.id, message.result)

        else:
            coro = self._handle_error(message.id, message.error)

        if inspect.iscoroutine(coro):
            self._create_task(coro, task_id=msg_id, name=f"{msg_type}: {msg_id}")
    ...
    def _create_task(
        ...
        task_id: json_rpc.MsgId,
        name: Optional[str] = None,
    ) -> asyncio.Task[Any]:
        """Create a task and ensure it gets cleaned up.

        Parameters
        ----------
        coro
           The co-routine to wrap in a task.

        task_id
           The unique id for this task.

        name
           An optional name to give to the task, if not set the ``task_id`` will be
           used.
        ...
        asyncio.Task[Any]
           The created task.
        """
        name = name or str(task_id)
        task = asyncio.create_task(coro, name=name)

        self._tasks[task_id] = task
        task.add_done_callback(partial(self._finish_task, task_id))

        return task

    def _finish_task(self, msg_id: json_rpc.MsgId, _t: asyncio.Task[Any]):
        """Cleanup a finished task."""
        if (task := self._tasks.pop(msg_id, None)) is not None:
            self.logger.debug("Task '%s' finished", task.get_name())
    ...
    def send_error(self, msg_id: json_rpc.MsgId, error: Any):
        ...
        return self._create_task(self._send(message), task_id=f"{msg_id}: send error")
    ...
    def send_notification(self, method: str, params: Any):
        ...
        task_id = f"{self.protocol.make_id()}: send notification"
        return self._create_task(self._send(message), task_id=task_id)
    ...
    def send_result(self, msg_id: json_rpc.MsgId, result: Any):
        ...
        return self._create_task(self._send(message), task_id=f"{msg_id}: send result")
    ...
    def send_request(
        ...
        -------
        asyncio.Task[Any]
           A task which, when complete, will contain the requst's result.
        """
        ...
        self._create_task(self._send(message), task_id=f"{msg_id}: send request")

        task_id = f"{msg_id}: response handler"
        task = self._create_task(self._wait_for_response(msg_id), task_id=task_id)

        if callback:
            task.add_done_callback(partial(_unwrap_task, callback))

        return task

    async def _wait_for_response(self, msg_id: json_rpc.MsgId):
        """Wait for a response to the given request id."""
        if msg_id not in self._futures:
            self.logger.error("Unknown message id: %s", msg_id)
            return

        result = await self._futures[msg_id]

        # Now that the response has been received, remove the future.
        self._futures.pop(msg_id)
        return result
    ...
    async def _execute_handler(self, handler, *args, **kwargs):
        ...
        # Some handlers may be async...
        if inspect.isawaitable(result):
            result = await result

        # Some may be generators
        elif inspect.isgenerator(result):
            result = await self._run_generator(result)

        return result

    async def _run_generator(self, gen):
    ...
    async def _handle_request(self, msg_id: json_rpc.MsgId, method: str, params: Any):
        ...
        if (handler := self._get_handler(method)) is None:
            ...
            await self.send_error(msg_id, error.to_response_error())
            return

        try:
            result = await self._execute_handler(handler, params)
        except JsonRpcException as exc:
            # Allow handlers to raise their own errors.
            await self.send_error(msg_id, exc.to_response_error())
            return
    ...
    async def _handle_notification(self, method: str, params: Any):
        ...
        try:
            await self._execute_handler(handler, params)
        except Exception as exc:
    ...
    def _error_handler(self, exc: Exception):
        try:
            res = self.error_handler(exc)

            if inspect.iscoroutine(res):
                msg_id = self.protocol.make_id()
                task = asyncio.create_task(res, name=f"Error handler for: {exc}")

                self._tasks[msg_id] = task
                task.add_done_callback(partial(self._finish_task, msg_id))
        except Exception:
...
def _unwrap_task(callback: Callable[[Any], None], task: asyncio.Task[Any]):
    """Call the given callback with the task's result."""
    callback(task.result())


async def rpc_main_loop(
    reader: asyncio.StreamReader,
    stop_event: threading.Event,
    message_handler: MessageHandler,
):
    """Main loop implemented using asyncio."""
