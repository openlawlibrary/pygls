...
def setup_ls_features(ls):
    ...
    async def cmd_test3(ls, *args):
        return True, threading.get_ident()

    @ls.thread()
    @ls.command(CMD_THREAD)
    def cmd_test1(ls, *args):
        return True, threading.get_ident()

    @ls.command(CMD_SYNC)
    def cmd_test2(ls, *args):
        return True, threading.get_ident()

    # Features
    @ls.feature(FEATURE_ASYNC)
    async def feature_async(ls, *args):
        return True, threading.get_ident()

    @ls.feature(FEATURE_SYNC)
    def feature_sync(ls, *args):
        return True, threading.get_ident()

    @ls.feature(FEATURE_THREAD)
    @ls.thread()
    def feature_thread(ls, *args):
        return True, threading.get_ident()
