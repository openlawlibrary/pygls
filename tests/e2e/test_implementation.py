...
async def test_implementation(
    ...
    implementation_options = initialize_result.capabilities.implementation_provider
    assert implementation_options is True
    ...
    response = await client.text_document_implementation(
    ...
    response = await client.text_document_implementation(
        types.ImplementationParams(
            text_document=types.TextDocumentIdentifier(uri=test_uri),
            position=types.Position(line=5, character=46),
        )
    )
