...
from pygls.jsonrpc import exceptions
from pygls.jsonrpc.endpoint import Endpoint
...
def test_consume_request_method_not_found(endpoint, consumer):
    ...
    assert_consumer_error(
        consumer, exceptions.JsonRpcMethodNotFound.of('methodName'))
...
def await_assertion(condition, timeout=3.0, interval=0.1, exc=None):
    ...
    try:
        ...
        time.sleep(interval)
        await_assertion(condition, timeout=(
            timeout - interval), interval=interval, exc=e)
